...
Implementation completion and controlled or stub-provider benchmark runs do not constitute acceptance. Only a separate human/operator-run live acceptance milestone using the frozen production candidate, current approved providers, the target workstation, real uncached operational measurements, and required blind relevance judgments may classify QMDX as accepted, rejected, or inconclusive.
